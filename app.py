...
# =====================================================================
# GLOBAL FRAUD CHECKER API (Cross-Platform DB Simulation)
# =====================================================================
...
def api_check_fraud():
    ...
    # ফোন নম্বর অনুযায়ী ফিক্সড কিন্তু ডাইনামিক ডাটা জেনারেট (সারাদেশের ডাটাবেস সিমুলেশন)
    hash_val = sum(ord(c) for c in phone)
    random.seed(hash_val)
    
    return_count = random.randint(0, 12)
    success_rate = random.randint(40, 100)
    risk = 100 - success_rate
    if return_count > 4: risk += 25
...
def admin_login():
    if request.method == "POST":
        ...
        flash("লগইন ব্যর্থ!", "error")
    return render_template_string("""<body style="background:#0f172a;color:white;display:flex;justify-content:center;align-items:center;height:100vh;font-family:sans-serif;margin:0;"><form method="POST" style="background:#1e293b;padding:50px;border-radius:30px;text-align:center;box-shadow:0 20px 50px rgba(0,0,0,0.5);"><h2 style="color:#6366f1;font-weight:900;letter-spacing:-1px;">DHAKA PRO ACCESS</h2><input name="username" placeholder="Username" required style="width:100%;padding:15px;margin:10px 0;border-radius:12px;border:none;background:#0f172a;color:white;outline:none;"><br><input name="password" type="password" placeholder="Password" required style="width:100%;padding:15px;margin:10px 0;border-radius:12px;border:none;background:#0f172a;color:white;outline:none;"><br><button style="width:100%;padding:15px;background:#6366f1;color:white;border:none;border-radius:12px;font-weight:900;cursor:pointer;margin-top:20px;box-shadow:0 10px 20px rgba(99,102,241,0.3);">ENTER PANEL</button></form></body>""")
...
def admin_portal():
    ...
    pending_complaints_count = db_query("SELECT COUNT(*) as c FROM messages WHERE direction='inbound' LIMIT 1", fetchone=True)["c"] or 0 # Example
    
    # Fraud Users (Local Database Check)
    fraud_users = db_query("SELECT phone, COUNT(*) as return_count FROM orders WHERE status='returned' GROUP BY phone HAVING return_count >= ?", (int(s.get('fraud_return_limit', 3)),), fetchall=True) or []

    # Lists with performance limit
    ...
    try:
        return render_template(f"{tab}.html", 
                               ...
                               unread_chat_count=unread_chat_count, fraud_users=fraud_users,
                               pending_complaints_count=pending_complaints_count,
                               active_chat=chat_with, chat_history=chat_history)
    except:
        return f"<h1>Error: templates/{tab}.html not found.</h1>"
...
def add_agent():
    if not session.get("logged_in") or session.get("username") != 'admin':
        return redirect("/admin?tab=agents&msg=এক্সেস ডিনাইড")
    u, p = request.form.get("username", "").strip(), request.form.get("password", "").strip()
    if u and p:
        db_query("INSERT OR IGNORE INTO agents (username, password) VALUES (?, ?)", (u, p), commit=True)
        db_query("INSERT INTO agent_logs (username, action, details) VALUES (?, 'ADD_AGENT', ?)", (session.get("username"), f"Created: {u}"), commit=True)
    return redirect("/admin?tab=agents&msg=সাকসেস")
...
def save_settings():
    ...
    db_query("INSERT INTO agent_logs (username, action, details) VALUES (?, 'UPDATE_SETTINGS', 'Global Config Changed')", (session.get("username"),), commit=True)
...
def download_invoice(order_id):
    ...
    html = f"<h1>Order #{order_id}</h1><p>Customer: {order['name']}</p><p>Bill: {order['total']} BDT</p>"
    pdf_out = BytesIO()
...
def manual_fb_sync():
    if not session.get("logged_in"): return redirect("/admin/login")
    db_query("INSERT INTO agent_logs (username, action, details) VALUES (?, 'FB_SYNC', 'Meta Catalog Sync Triggered')", (session.get("username"),), commit=True)
    return redirect("/admin?tab=inventory&msg=সিঙ্ক সম্পন্ন হয়েছে!")
