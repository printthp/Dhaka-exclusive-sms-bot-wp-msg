...
# =====================================================================
# SYSTEM & STORAGE SETUP (Render Fixed)
# =====================================================================
...
# =====================================================================
# PATHAO INTEGRATION (FORCEFUL AUTO-TOKEN)
# =====================================================================
...
def export_excel():
    ...
    output.seek(0)
    return send_file(output, as_attachment=True, download_name=f"Full_Report_{datetime.now().strftime('%Y-%m-%d')}.xlsx")
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            if p_id:
                ...
                count += 1
        return redirect("/admin?tab=orders&msg=Success Import")
...
def admin_portal():
    ...
    chat_history = db_query("SELECT * FROM messages WHERE from_number=? ORDER BY id DESC", (chat_with,), fetchall=True) or [] if chat_with else []
    ...
    return render_template(f"{tab}.html", settings=s, analytics=analytics, orders=orders, users=users, chat_history=chat_history[::-1], active_chat=chat_with, msg=msg)

@app.route("/admin/sync-pathao-status")
def sync_trigger():
    return redirect(url_for('admin_portal', tab='orders', msg=f"Process Result: {pull_orders_from_pathao()}"))

@app.route("/admin/login", methods=["GET", "POST"])
def login():
    if request.method == "POST":
        if request.form.get("username") == "admin" and request.form.get("password") == "admin123":
            session["logged_in"], session["username"] = True, "admin"
            return redirect("/admin")
    return render_template_string('<body style="background:#020617;color:white;display:flex;justify-content:center;align-items:center;height:100vh;"><h2>DHAKA PRO LOGIN</h2><form method="POST"><input name="username" placeholder="User"><br><input name="password" type="password" placeholder="Pass"><br><button>ENTER</button></form></body>')

@app.route("/api/check-fraud")
def api_fraud():
    phone = request.args.get("phone")
    return jsonify({"phone": phone, "risk": random.randint(0, 100)})
