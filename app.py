...
# =====================================================================
# FEATURE: REAL PATHAO API SYNC (PULLS REAL DATA)
# =====================================================================
...
def pull_orders_from_pathao():
    ...
    if not token or not store_id:
        logger.warning("Pathao API keys missing in settings. Real sync skipped.")
        return 0
    ...
    try:
        ...
        pathao_orders = r.json().get('data', {}).get('data', [])
        
        pulled = 0
        for o in pathao_orders:
            # ইউনিক পাঠাও অর্ডার আইডি চেক করে ডাটাবেসে সেভ করা
            res = db_query("""
                INSERT OR IGNORE INTO orders (pathao_order_id, phone, name, address, total, status) 
                VALUES (?, ?, ?, ?, ?, ?)
            """, (str(o['consignment_id']), o['recipient_phone'], o['recipient_name'], o['recipient_address'], o['amount'], o['status']), commit=True)
            if res: pulled += 1
        return pulled
    except Exception as e:
        logger.error(f"Pathao API Call Error: {e}")
        return 0
...
def admin_login():
    if request.method == "POST":
        ...
        if auth:
            ...
            return redirect("/admin?tab=dashboard&msg=Logged in successfully")
    return render_template_string('<body style="background:#0f172a;color:white;display:flex;justify-content:center;align-items:center;height:100vh;font-family:sans-serif;"><form method="POST" style="background:#1e293b;padding:40px;border-radius:20px;text-align:center;"><h2>DHAKA PRO LOGIN</h2><input name="username" placeholder="User" style="width:100%;padding:10px;margin:10px 0;"><br><input name="password" type="password" placeholder="Pass" style="width:100%;padding:10px;margin:10px 0;"><br><button style="width:100%;padding:10px;background:#6366f1;color:white;border:none;margin-top:20px;">LOGIN</button></form></body>')
...
def admin_portal():
    ...
    # ড্যাশবোর্ড রেন্ডার করার আগে রিয়েল ডাটা পুল করা
...
def sync_pathao_status():
    if not session.get("logged_in"): return redirect("/admin/login")
    new_orders = pull_orders_from_pathao()
    return redirect(url_for('admin_portal', tab='dashboard', msg=f"Pathao Sync Complete! {new_orders} new orders found."))
