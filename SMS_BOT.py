...
# --- Gemini AI Setup (Updated) ---
genai.configure(api_key=GEMINI_KEY)
# models/ অংশটি যোগ করা হয়েছে যাতে ৪0৪ এরর না আসে
model = genai.GenerativeModel('gemini-1.5-flash')

def get_ai_answer(user_query):
    try:
        context = "You are the assistant for 'Dhaka Exclusive', a kitchenware shop in Bangladesh. Answer politely in Bengali."
        response = model.generate_content(f"{context}\nCustomer: {user_query}")
        return response.text
    except Exception as e:
        print(f"AI Error: {e}")
        return "দুঃখিত, আমি এখন উত্তর দিতে পারছি না। একটু পরে চেষ্টা করুন।"
