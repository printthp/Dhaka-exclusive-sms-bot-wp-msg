...
# =====================================================================
# ⚙️ ১. ENV সিক্রেটস (সব default মান নিয়ে, মিস হলে crash না)
# =====================================================================
...
APP_SECRET = os.environ.get("APP_SECRET", "378c339366554565e35bc64dc6601a56")  # খালি থাকলেও কাজ করবে

ADMIN_NUMBERS_STR = os.environ.get("ADMIN_NUMBERS", "01717121068")
...
PATHAO_MERCHANT_PASSWORD = os.environ.get("PATHAO_MERCHANT_PASSWORD", "cocid1000006@gmail.com")
...
# =====================================================================
# 🛡️ ৪. Webhook Verify (APP_SECRET না থাকলে skip — crash না!)
# =====================================================================
def verify_meta_signature(payload, signature):
    if not APP_SECRET:
        return True  # খালি থাকলে verify skip
    if not signature:
        return False
    try:
...
def api_post_retry(url, payload, headers, max_retries=3):
    for attempt in range(max_retries):
        try:
            ...
            if res.status_code >= 500: time.sleep(2 ** attempt); continue
            return res
...
def get_pathao_token():
    ...
    if not res: return None, "Network error"
    data = res.json()
...
def create_pathao_order(name, phone, address, city_id=1, zone_id=1, area_id=1, item_desc="Premium Kitchenware", cod_amount=0):
    ...
    res = api_post_retry(url, payload, headers)
    if not res: return False, "API timeout"
    data = res.json()
    if res.status_code == 200 and data.get("status") == 200:
...
def send_text(to, body):
    ...
    try:
        ...
        logger.error(f"Send text error: {e}"); return False
...
def send_buttons(to, body, buttons):
    ...
    try:
        ...
        logger.error(f"Send buttons error: {e}"); return False
...
def send_list_menu(to, body, button_text, sections):
    ...
    try:
        ...
        logger.error(f"Send list error: {e}"); return False
...
def send_image(to, image_url, caption=""):
    ...
    headers = {"Authorization": f"Bearer {PERMANENT_TOKEN}", "Content-Type": "application/json"}
    payload = {
        "messaging_product": "whatsapp", "to": format_phone(to), "type": "image",
    ...
    try:
        ...
        logger.error(f"Send image error: {e}"); return False
...
def validate_coupon(code):
    ...
    if not row: return None, "কুপন সঠিক নয়।"
    if row["used_count"] >= row["max_uses"]: return None, "কুপন শেষ।"
    if row["valid_until"] and datetime.now().isoformat() > row["valid_until"]: return None, "মেয়াদ শেষ।"
    return row, None
...
def process_webhook_async(msg, from_number):
    ...
    if user_text.lower().startswith("admin:"):
        ...
        if cmd.lower().startswith("coupon"):
            ...
            if len(parts) >= 5:
                ...
                db_query("INSERT INTO coupons (code, discount_percent, discount_amount, max_uses, valid_until) VALUES (?, ?, ?, ?, ?)",
    ...
    if state == "selecting_city":
        if user_text.startswith("city_"):
            ...
            if zones:
                sections = [{
                    "title": "জোন",
                    "rows": [{"id": f"zone_{z['zone_id']}", "title": z['zone_name'][:24]} for z in zones[:10]]
                }]
    ...
    if state == "selecting_zone":
        if user_text.startswith("zone_"):
            ...
            if areas:
                sections = [{
                    "title": "এরিয়া",
                    "rows": [{"id": f"area_{a['area_id']}", "title": a['area_name'][:24]} for a in areas[:10]]
                }]
    ...
    if state == "selecting_payment":
        if user_text.lower() in ["cod", "cash", "ক্যাশ", "ক্যাশঅনডেলিভারি"]:
            ctx = get_context(from_number)
            ctx["payment_method"] = "cod"; ctx["delivery_charge"] = 80
        ...
        if user_text.lower() in ["bkash", "বিকাশ"]:
            ctx = get_context(from_number)
            ctx["payment_method"] = "bkash"; ctx["delivery_charge"] = 80
            ctx["total"] = ctx["subtotal"] + 80
            set_session(from_number, "awaiting_coupon", context=ctx)
        ...
        if user_text.lower() in ["nagad", "নগদ"]:
            ctx = get_context(from_number)
            ctx["payment_method"] = "nagad"; ctx["delivery_charge"] = 80
            ctx["total"] = ctx["subtotal"] + 80
            set_session(from_number, "awaiting_coupon", context=ctx)
    ...
    if state == "awaiting_coupon":
        ...
        if user_text.lower() in ["নেই", "no", "nope", "কুপন নেই"]:
            ctx["coupon"] = None; ctx["discount"] = 0
        else:
        ...
        if user_text in ["হ্যাঁ", "yes", "confirm_yes", "✅ হ্যাঁ"]:
            ...
            if success:
                db_query(
                    ...
                    commit=True)
                if ctx.get("coupon"): use_coupon(ctx["coupon"])
    ...
    if any(k in user_text.lower() for k in ["কিনব", "অর্ডার", "চাই", "buy", "order", "দাম"]):
        ...
        if products:
            sections = [{
                "title": "প্রোডাক্ট",
                "rows": [{"id": f"product_{p['id']}", "title": p['name'][:24], "description": f"{p['price']}৳"}
                         for p in products[:10]]
            }]
            set_session(from_number, "selecting_product", context={})
...
def home():
    return jsonify({"status": "running", "service": f"{BUSINESS_NAME} WhatsApp Bot",
                    "version": "3.0", "business_hours": BUSINESS_HOURS,
                    "timestamp": datetime.utcnow().isoformat()})

@app.route("/health", methods=["GET"])
def health():
    try: db_query("SELECT 1", fetchone=True); db_ok = True
    except: db_ok = False
